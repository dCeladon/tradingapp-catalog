...
from supabase_client import fetch_backtests_and_count  # ← usa la nuova funzione
...
def parse_all_metrics(perf):
    ...
    Supporta JSON annidato, JSON piatto (metrics.*) e summary.
    """
    if perf is None:
        return ("—", "—", "—", "—", "—")
    ...
    return (
        fmt_num(net_profit, nd=2),
        fmt_num(profit_factor, nd=2),
        fmt_num(max_dd, nd=2),
        fmt_pct(win_rate),
        fmt_int(n_trades)
    )
...
- consulenza in materia di investimenti, raccomandazioni personalizzate o ricerca in materia di investimenti ai sensi di **MiFID II**;
...
# -------- stato pagina e offset --------
page = int(st.session_state.get("page", 1))
page = max(1, page)
offset = (page - 1) * PAGE_SIZE

# -------- Fetch con conteggio totale --------
items, total_count = fetch_backtests_and_count(limit=PAGE_SIZE, offset=offset)
total_pages = max(1, math.ceil(total_count / PAGE_SIZE)) if total_count else 1

# Se l'utente è finito oltre l'ultima pagina (es. dopo cambi di dati), clamp e ricarica
if page > total_pages:
...
# -------- Paginazione (TOP) con totale reale --------
render_pagination_controls(total_pages=total_pages, key_prefix="top")

# -------- Render griglia --------
if total_count == 0:
    st.info("Nessun backtest pubblicato al momento.")
else:
...
# -------- Paginazione (BOTTOM) con totale reale --------
render_pagination_controls(total_pages=total_pages, key_prefix="bottom")
